...
import { Trophy, Play, RotateCcw, CheckCircle2, XCircle, User, Waves, Settings, Plus, Trash2, Save, X, Info, Pause, PlayCircle } from 'lucide-react';
...
export default function App() {
  ...
  const nextQuestion = useCallback(() => {
    ...
    if (availableIndices.length === 0) {
      // All questions used! End the game.
      if (gameMode === 'time-trial') {
        endGame(selectedTeam);
      } else {
        // In head-to-head, judge by current position
        if (posA > posB) endGame('A');
        else if (posB > posA) endGame('B');
        else endGame('DRAW'); // Or just end game with current leaderboard
      }
      return;
    }
  ...
  }, [questionsList, usedIndices, questionDuration, gameMode, selectedTeam, posA, posB]);
  ...
  useEffect(() => {
    if (gameState === 'playing' && gameMode !== 'time-trial' && questionDuration > 0 && !isPaused) {
      if (questionTimeLeft > 0) {
        ...
        // Time's up!
        playSound('wrong');
        setFeedback({ text: 'Hết thời gian suy nghĩ!', type: 'error' });
  ...
  }, [gameState, questionTimeLeft, isPaused, questionDuration, gameMode, teamATurn, currentIndex]);
  ...
  const handleAnswer = (e?: React.FormEvent) => {
    ...
    if (isCorrect) {
      ...
      if (gameMode === 'time-trial') {
        ...
        setTimeout(() => {
          setFeedback({ text: '', type: null });
          nextQuestion(); // Get a NEW question for the other team
          setTeamATurn(!teamATurn);
          setIsProcessing(false);
        }, 1200);
      }
  ...
  <h1 className="text-lg font-bold text-orange-600 tracking-tight uppercase">
    ...
    <div className="flex gap-2 items-center">
      {gameMode === 'time-trial' && gameState === 'playing' && (
        <div className={`flex items-center gap-2 px-3 py-1 rounded-full border-2 transition-all ${isPaused ? 'bg-gray-100 border-gray-400 opacity-60' : 'bg-orange-100 border-orange-500 animate-pulse'}`}>
          <span className={`${isPaused ? 'text-gray-500' : 'text-orange-600'} font-black text-sm`}>
            {isPaused ? '⏸️ TẠM DỪNG' : `⏱️ ${elapsedTime} GIÂY`}
          </span>
        </div>
      )}
    ...
    {gameState === 'playing' && (
      <button 
        ...
        className={`p-1.5 rounded-full transition-all flex items-center justify-center ${isPaused ? 'bg-green-500 text-white animate-pulse' : 'text-gray-500 hover:text-orange-600 hover:bg-orange-50'}`}
    ...
    {gameMode !== 'time-trial' && (
      ...
      >
        ...
        <div className="min-h-[180px] p-3 flex flex-col items-center justify-center gap-2 z-20">
          ...
          {gameState === 'playing' && (
            <div className="w-full max-w-2xl flex flex-col items-center gap-2 px-4 relative">
              {/* Pause Overlay */}
              <AnimatePresence>
                {isPaused && (
                  <motion.div 
              ...
              <div className="text-center flex flex-col items-center gap-2 w-full z-20">
                ...
                <div className="p-3 w-full flex flex-col items-center justify-center min-h-[100px]">
              ...
              <form onSubmit={handleAnswer} className="flex gap-2 w-full max-w-md">
                <input
                  ...
                  placeholder={(gameMode === 'ai' && !teamATurn) || isProcessing || isPaused ? "Đợi..." : "Đáp án..."}
            ...
            >
              <div className="flex flex-col items-center text-center">
                <div className="flex flex-col items-center gap-2 text-xl font-black text-orange-600">
                  ...
                  {gameMode === 'time-trial' ? (
                    <span>HOÀN THÀNH! THỜI GIAN: {elapsedTime} GIÂY</span>
                  ) : winner === 'DRAW' ? (
                    <span>HÒA NHAU! CẢ HAI ĐỀU RẤT GIỎI!</span>
                  ) : (
            ...
            >
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 bg-orange-50 p-4 rounded-2xl border-2 border-orange-100">
                  ...
                  {/* Time Setting */}
                  <div className="sm:col-span-2 space-y-4 pt-2 border-t border-orange-200">
                    <div className="space-y-2">
                      <label className="text-xs font-black text-orange-600 uppercase">Thời gian tối đa để trả lời một câu hỏi (Giây)</label>
                      <p className="text-[10px] text-gray-500 italic mt-0.5">Áp dụng cho chế độ 2 Người chơi & Đấu với Máy. Nhập 0 để không giới hạn.</p>
                      <div className="flex items-center gap-3">
                        <div className="flex items-center gap-2 flex-1">
                          <input 
                            ...
                            className={`w-full px-4 py-2.5 rounded-xl font-black text-lg outline-none transition-all ${questionDuration > 0 ? 'bg-orange-500 text-white focus:ring-4 focus:ring-orange-200' : 'bg-gray-100 text-gray-400 focus:bg-white focus:ring-4 focus:ring-gray-100'}`}
                          />
                          <span className="text-orange-600 font-black">GIÂY</span>
                        </div>
                        {questionDuration > 0 && (
                          <button 
                            onClick={() => setQuestionDuration(0)}
                            className="bg-gray-200 text-gray-500 px-3 py-2.5 rounded-xl font-bold text-xs hover:bg-gray-300 transition-colors"
                          >
                            XÓA GIỚI HẠN
                          </button>
                        )}
